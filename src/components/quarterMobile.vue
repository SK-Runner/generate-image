  ...
  methods: {
    ...
    save (num) {
      return new Promise((resolve, reject) => {
        console.log('here')
        const quarterMobile = document.getElementById('quarterMobile')
        ...
        html2canvas(quarterMobile, opts).then(res => {
          // const { height, width } = res
          const base64 = res.toDataURL('image/png', 1)
          this.Download(base64, `quarterMobile${num}.png`)
          resolve()
        }, () => {
          alert('截图失败，请重新尝试')
