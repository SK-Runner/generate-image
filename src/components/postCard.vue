<template>
  <div>
    ...
    <div id="post" :class="{'year':type==2,'quarter':type==1,'month':type==0}">
      <span class="number">{{No}}</span>
      <img class='image' :class="{'image-top':type!=2}" :src='image'>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    save () {
      ...
      html2canvas(post, opts).then(res => {
...
.year {
  background-image: url("./../assets/bg/post-year.jpg");
}
.quarter {
  background-image: url("./../assets/bg/post-quarter.jpg");
}
.month {
  background-image: url("./../assets/bg/post-month.jpg");
}
.number {
  position: absolute;
  right: 10px;
